...
function computeVeMGM(amount, locktime, ts) {
...
function getVeStats(value, locktime, ts) {
    ...
    const ve = computeVeMGM(amount, parseInt(locktime), parseInt(ts));
    const days = parseInt((locktime - ts) / DAY);
...
async function exec_holder_info() {
    ...
    veNftStats = stats;
    holderInfo = balances;
    console.log(`- Holder info: ${balances.length}, ${currency(stats.veAmount)} veNFT, ${currency(stats.tokensAmount)} MGM.`);
